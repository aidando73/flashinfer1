    ...
    if constexpr (!COMPILE_HOPPER_TMA_GROUPED_GEMMS_ENABLED &&                                                                                                                                                                                                                               \
      ...
      static_assert(cutlass::platform::is_same<T, WeightType>::value || IsWFP4AFP8,                                                                                                                                                                                                          \
                    "TMA warp specialized MOE implementation does not support mixed input types");                                                                                                                                                                                           \
                                                                                                                                                                                                                                                                                             \
// Block-scaled (MXFPX / NVFP4) modes use CUTLASS block-scaled tensor ops on Blackwell.
// - NVFP4: T == FP4 (A is block-scaled)
// - WFP4AFP8: WeightType == FP4 and T == FP8 (B is block-scaled, A may or may not be)
// - MXFP8@MXFP8: T == FP8 and WeightType == FP8 (A and B are block-scaled) -> IsMXFPX=true
constexpr static bool IsBlockScaled = IsMXFPX || IsFP4 || IsWFP4AFP8;
...
/* using ElementC = std::conditional_t<BIAS, ElementType, void>; */ /* using ElementCSafe =
                                                                       std::conditional_t<BIAS,
                                                                       ElementType, ElementD>;
                                                                     */
using ElementC = void;
...
using ElementSF =
    std::conditional_t<IsMXFPX, cutlass::float_ue8m0_t,
                       cutlass::float_ue4m3_t>; /*TmaWarpSpecializedGroupedGemmInput::ElementSF;*/
using ElementActBlockScaled = std::conditional_t<
    IsSM120,
...
constexpr static int AlignmentWeight =
    IsWFP4AFP8
        ? 128
        : (128 / cutlass::sizeof_bits<ElementWeight>::value); /* Memory access \
                                                        granularity/alignment of B matrix in units \
                                                        // of elements (up to 16 bytes)*/

/* C matrix configuration */ /* Note we use ElementType here deliberately, so we don't break
...
static_assert(cutlass::platform::is_same<
                  EpilogueTag, tensorrt_llm::cutlass_extensions::EpilogueOpDefault>::value,
              "TMA Warp Specialized Grouped GEMM specialisation doesn't support fused activation");

using EpilogueOp = cutlass::epilogue::fusion::LinearCombination<ElementD, ElementAccumulator,
...
using EpilogueSchedule =
    std::conditional_t<IsSM10x, EpilogueScheduleSM10x,
...
using EpilogueTensorOp =
    std::conditional_t<IsBlackwell && IsBlockScaled, cutlass::arch::OpClassBlockScaledTensorOp,
                       cutlass::arch::OpClassTensorOp>;
using EpilogueScheduleSM10xFinalize =
    std::conditional_t<!IsFinalizeFusion && IsSM10x,
                       std::conditional_t<Is2SM, cutlass::epilogue::PtrArrayTmaWarpSpecialized2Sm,
                                          cutlass::epilogue::PtrArrayTmaWarpSpecialized1Sm>,
                       EpilogueSchedule>; /* This still needs to be valid when finalize fusion is
                                             disabled */
...
using KernelScheduleSM100 =
    std::conditional_t<Is2SM,
...
using KernelScheduleSM103 = std::conditional_t<
    Is2SM, cutlass::gemm::KernelPtrArrayTmaWarpSpecialized2SmBlockScaledMxNvf4UltraVs16Sm103,
...
if (workspace_size != nullptr) { /* Make a mock problem shape with just the minimal information
                                 actually required to get \
                                 the workspace \
                                 // size This makes some assumptions about CUTLASS's implementation
                                 which is suboptimal. We \
                                 have a check \
                                 // later to catch future cutlass updates causing silent breakages,
                                 but that is not fool \
                                 proof. The \
                                 // alternative is to wait until we have data and then dynamically
...
EpilogueScalars epilogue_scalars = [&] {
  ...
  if constexpr (IsFinalizeFusion) {
    ...
    return construct_if_true<(!IsSimpleAlphaBeta && !IsFinalizeFusion), EpilogueScalars>(
        ...
        cute::Shape<_0, _0, int64_t>{cute::_0{}, cute::_0{},
